...
//这个是分割线，基本上不要动就好。如果实在觉得这个分割线丑的话，你可以在第20-32行修改制定自定义样式

public class TicketDivideLine extends View {

    //分割线默认颜色
    private final int DIVIDELINE_DEFAULT_COLOR = 0xFFA3A3A3;
    //两端半圆默认颜色
    private final int PORTSHAPE_DEFAULT_COLOR = 0xFFf1f2f6;
    //两端半圆默认高度(直径)
    private final int PORTSHAPE_DEFAULT_DIAMETER = dp2px(14);
    //分割线默认长度
    private final int DIVIDELINE_DEFAULT_LENGTH = dp2px(2);
    //分割线默认间隔长度
    private final int DIVIDELINE_DEFAULT_INTERVAL = dp2px(4);
    //分割线默认方向orientation
    ...
    public TicketDivideLine(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        mDivideLineColor = ta.getColor(R.styleable.TicketDivideLine_divide_line_color, DIVIDELINE_DEFAULT_COLOR);
        ...
        mDivideLineOrientation = ta.getInt(R.styleable.TicketDivideLine_line_orientation, DIVIDELINE_DEFAULT_ORIENTATION);
